...
- The watchdog is \`$WATCHDOG_TIMER\` plus its oneshot service and script \`$WATCHDOG_SCRIPT\`. It is a DSM systemd timer, not a separate always-running process.
- DSM starts the watchdog after boot and then once per minute. It requires the timer to be active/enabled, DSM UPS support to be enabled, \`ups-usb.service\` to expose \`ups@localhost\`, and \`upsc ups@localhost ups.status\` to return a valid UPS state.
- On each run, the watchdog reads UPS status plus DSM's current \`ups_wait_time\` and \`ups_safeshutdown\` values. If the UPS remains on battery for the configured wait time, it sends the UPS shutdown-return command as root while DSM is still fully running, then calls Synology's low-battery Safe Mode path.
