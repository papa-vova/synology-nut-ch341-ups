...
#   UPS_OFF_DELAY_SECONDS   NUT output-cut delay for UPSes that honor shutdown-return.
#   UPS_ON_DELAY_SECONDS    NUT output-restore delay for UPSes that honor shutdown-return.
...
.PHONY: help require-nas require-module deps build install restore check probe
...
restore: require-nas
	$(SCP) scripts/synology-ch341-ups-install.sh "$(SSH_TARGET):/tmp/"
	$(SUDO_SSH) "$(SSH_TARGET)" "sudo /bin/sh /tmp/synology-ch341-ups-install.sh restore"
